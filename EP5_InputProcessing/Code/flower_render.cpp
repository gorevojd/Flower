...
INTERNAL_FUNCTION void PushDefaultMatricesToRectBuffer(render_params* Params, rect_buffer* RectBuffer)
{
    ...
    RectBuffer->OrthoMatrixIndex = PushRectTransform(RectBuffer, Params->ScreenOrthoProjection);
    
    // NOTE(Dima): Pushing screen perspective projection matrix
    RectBuffer->ViewProjMatrixIndex = PushRectTransform(RectBuffer, Params->ViewProjection);
}
...
INTERNAL_FUNCTION void BeginRender(render_params* Params)
{
    ...
    PushDefaultMatricesToRectBuffer(Params, &Commands->Rects2D);
    PushDefaultMatricesToRectBuffer(Params, &Commands->Rects3D);
}
