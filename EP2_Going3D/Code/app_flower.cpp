/*
TODO(Dima): list of stuff that we can potentially fix and do

Episode This:
- Virtualize input buttons
- Create Horizontal/Vertecal, MouseX/MouseY input axes;

Episode Later:
- Render queues
- Memory arenas
- Dynamically growable arenas
...
int main(int ArgsCount, char** Args)
{
    ...
    while(App.Running)
    {
        ...
        Input.AppDeltaTime = App.DeltaTime;
        Input.AppTime = App.Time;
        ...
        {
            SDL_Log("Button Left is pressed\n");
        }
        
        
        if(GetButtonDown(Button_Right))
        {
            SDL_Log("Button Right is pressed\n");
        }
        
        if(GetButtonUp(Button_Up))
        {
            SDL_Log("Button Up is pressed\n");
        }
        
        if(GetButtonUp(Button_Down))
        {
            SDL_Log("Button Down is pressed\n");
        }
        
        if(GetButtonDown(Button_Jump))
        {
            SDL_Log("Button Jump is pressed\n");
        }
#endif
        ...
        v3 MoveVector = NOZ(V3(Moves.x, 0.0f, Moves.z)) * Input.AppDeltaTime;
