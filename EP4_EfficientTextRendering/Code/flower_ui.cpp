inline f32 GetKerning(font* Font, u32 CodepointFirst, u32 CodepointSecond)
{
...
INTERNAL_FUNCTION void PrintText_(render_commands* Commands, 
                                  font* Font, 
                                  char* Text, 
                                  v3 Left, v3 Up,
                                  v3 P, 
                                  ...
                                  v4 C = V4(1.0f, 1.0f, 1.0f, 1.0f))
{
    ...
    glyph_buffer* Buffer = &Commands->Glyphs2D;
    
    int IndexToTransformMatrix = Buffer->GlyphDefaultProjectionMatrixIndex;
    
    b32 Is3D = (Flags & PrintText_3D) != 0;
    if(Is3D)
    {
        Buffer = &Commands->Glyphs3D;
        AtP = {};
        ...
        m44 TextTransform = MatrixFromRows(V4(Left, 0.0f), 
                                           V4(Up, 0.0f), 
                                           V4(NOZ(Cross(Left, Up)), 0.0f),
                                           V4(P.x, P.y, P.z, 1.0f));
        IndexToTransformMatrix = PushGlyphTransform(Buffer, &TextTransform);
    }
...
INTERNAL_FUNCTION void PrintText(render_commands* Commands, 
                                 font* Font, 
                                 char* Text, 
                                 v2 P, 
                                 f32 Scale = 1.0f, 
                                 ...
                                 b32 WithShadow = true)
{
    ...
    PrintText_(Commands, Font, Text, V3_Left(), V3_Up(), V3(P, 0.0f), V2(0.0f, 0.0f), 0, Scale, C);
}
...
INTERNAL_FUNCTION void Text(char* Txt)
{
    Assert(GlobalUI);
    
    ui_params* Params = &GlobalUI->Params;
    
    PrintText(Params->Commands, 
              Params->Font, 
              Txt, 
              GlobalUI->CurrentLayout->At, 
              Params->Scale, V4(1.0f, 1.0f, 1.0f, 1.0f));
